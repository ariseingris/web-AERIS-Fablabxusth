...
module.exports = function attachMqttBridge(httpServer) {
  ...
  async function persistAerisSensorData(deviceId, payload) {
    const reading = {
      ...
      temperature_raw: payload.raw?.temp ?? null,
      humidity_raw:    payload.raw?.hum ?? null,
      co2_raw:         payload.raw?.co2 ?? null,
      ch4_raw:         payload.raw?.ch4 ?? null,
      pressure_raw:    payload.raw?.pressure ?? null,
      light_raw:       payload.raw?.lux ?? null,
  ...
  function persistSensorData(deviceId) {
    ...
    persistTimers.set(deviceId, setTimeout(async () => {
      ...
      const reading = {
        ...
        temperature_raw: sensors.temperature?.raw ?? null,
        humidity_raw:    sensors.humidity?.raw ?? null,
        co2_raw:         sensors.co2?.raw ?? null,
        ch4_raw:         sensors.ch4?.raw ?? null,
        pressure_raw:    sensors.pressure?.raw ?? null,
        light_raw:       sensors.light?.raw ?? null,
      };
